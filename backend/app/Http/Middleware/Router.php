        ...
        if ($route === false) {
            ...
            if ($failed_route instanceof Route) {
                ...
                if ($failed_route->failedRule === Accepts::class) {
                    file_put_contents('/tmp/heritago_debug.log', "FAILED RULE: Accepts | Needed: " . implode(',', $failed_route->accepts) . " | Got: " . $request->getHeaderLine('Accept') . "\n", FILE_APPEND);
                    return Registry::responseFactory()->response('Negotiation failed', StatusCodeInterface::STATUS_NOT_ACCEPTABLE);
                }

                file_put_contents('/tmp/heritago_debug.log', "FAILED RULE: " . $failed_route->failedRule . "\n", FILE_APPEND);
            }

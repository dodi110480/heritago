...
class TreeService
{
    ...
    public function create(string $name, string $title, string $firstName, string $lastName, string $gender, string $birthDate): Tree
    {
    ...
    /**
     * Find the next available XREF ID for a record type (INDI, FAM, etc.)
     */
    public function nextXref(Tree $tree, string $tag): string
    {
        // error_log("nextXref called for $tag tree=" . $tree->id());
        $table = match ($tag) {
            'INDI' => 'individuals',
            'FAM' => 'families',
            'SOUR' => 'sources',
            ...
            'OBJE' => 'media',
            'NOTE' => 'notes',
            default => throw new DomainException('Invalid tag for nextXref: ' . $tag),
        };

        $prefix = substr($tag, 0, 1); // I, F, S, R, O, N
        if ($tag === 'OBJE')
            $prefix = 'M';

        // simplistic approach: find max ID
        // In a real scenario, webtrees tracks this in `gedcom_setting` or similar, 
        // but for now we'll query the table.
        // IDs are like I1, I2, I100. We need to extract the number.

        $fileCol = strtolower($prefix) . '_file';

        $rows = DB::table($table)
            ->where($fileCol, '=', $tree->id())
            ->get();

        $maxId = $rows->map(function ($row) use ($prefix) {
            // Determine column name dynamically: i_id, f_id, etc.
            // Webtrees uses lowercase prefixes for columns e.g. i_id, f_id
            $colName = strtolower($prefix) . '_id';
            $id = $row->$colName ?? $row->{strtoupper($prefix) . '_ID'} ?? null;

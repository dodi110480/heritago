...
export function LinkBreakIconWrapper({ d, card_dim }: { d: TreeDatum, card_dim: CardDim }) {
  let g = "",
    r = d.data.rels, _r = (d.data as any)['_rels'] || {},
    closed = (d.data as any)['hide_rels'],
    areParents = (r: TreeDatum['data']['rels']) => r.parents.length > 0,
    areChildren = (r: TreeDatum['data']['rels']) => r.children && r.children.length > 0
  if ((d.is_ancestry || (d.data as any)['main']) && (areParents(r) || areParents(_r))) { g += LinkBreakIcon({ x: card_dim.w / 2, y: 0, rt: -45, closed }).template }
  if (!d.is_ancestry && d.added) {
    const sp = d.spouse!, sp_r = sp.data.rels, _sp_r = (sp.data as any)['_rels'] || {};

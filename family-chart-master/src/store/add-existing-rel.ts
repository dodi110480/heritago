...
export function getLinkRelOptions(datum: any, data: any) {
  const rel_datum = datum['_new_rel_data'] ? data.find((d: any) => d['id'] === datum['_new_rel_data'].rel_id) : null
  ...
  return data.filter((d: any) => d['id'] !== datum['id'] && d['id'] !== rel_datum?.id && !d['_new_rel_data'] && !d['to_add'] && !d['unknown'])
    .filter((d: any) => !ancestry_ids.includes(d.id))
    .filter((d: any) => !progeny_ids.includes(d.id))
    .filter((d: any) => !(d.rels.spouses || []).includes(datum.id))

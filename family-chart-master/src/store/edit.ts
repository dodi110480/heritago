...
export function submitFormData(datum: Datum, data_stash: Data, form_data: FormData) {
  ...
  if ((datum as any)['to_add']) delete (datum as any)['to_add']
  if ((datum as any)['unknown']) delete (datum as any)['unknown']
}
...
export function cleanupDataJson(data: Data) {
  ...
  data.forEach(d => {
    delete (d as any)['main']
    delete (d as any)['_tgdp']
    delete (d as any)['_tgdp_sp']
    delete (d as any)['__tgdp_sp']
  })
  data.forEach(d => {
...
export function removeToAddFromData(data: Data) {
  for (let i = data.length - 1; i >= 0; i--) {
    if ((data[i] as any)['to_add']) removeToAdd(data[i], data)
  }
}